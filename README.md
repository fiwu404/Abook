...
打开 <http://localhost:5173>。首次用 `admin/admin12345678` 登录。admin 固定拥有全部权限，并可在“账号与权限”创建教师、学生账号。创建时可逐项勾选教材、知识点、试题、试卷、任务、模型、账号、考试及练习权限，创建后可修改用户名和权限，也可重置初始密码；重置会使该用户原有登录状态立即失效。勾选“登录后必须修改密码”后，用户只能先完成密码修改，再进入工作台。管理类权限会自动包含完成该工作所需的查看权限。默认教师可阅读教材与已发布试卷，默认学生还可参加考试、查看成绩和错题练习。系统不开放自助注册。部署前修改 `.env` 中的数据库密码、`ADMIN_PASSWORD` 和 `APP_SECRET`。
...
试卷草稿可直接从列表删除。已发布试卷可由 admin 修改考试时间；已有答卷时只能调整结束时间。admin 可提前终止考试，未交卷答卷会立即按已保存答案判分。已发布试卷自然结束或被终止后才能从列表删除；删除保留试卷快照、审核记录和历史答卷。
