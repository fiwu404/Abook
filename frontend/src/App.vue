...
const isStaff = computed(() => user.value && user.value.role !== 'student')
...
async function login() {
  const data = await act(() => api(registerMode.value ? '/auth/register' : '/auth/login', 'POST', auth.value), '登录成功')
...
async function refresh() {
  ...
  if (isStaff.value) jobs.value = await api('/jobs')
  else { const [a, m, v] = await Promise.all([api('/my/attempts'), api('/my/mastery'), api('/my/variant-requests')]); attempts.value = a; mastery.value = m; variantRequests.value = v }
...
<template>
  <div class="shell">
    <header class="topbar"><div class="brand"><span class="brand-mark">A</span><div><strong>Abook</strong><small>教材 · 知识 · 考试</small></div></div><div v-if="user" class="account"><span class="role-tag">{{ user.role === 'operator' ? '教务操作' : user.role === 'reviewer' ? '内容审核' : '考生' }}</span><span>{{ user.username }}</span><button class="text-btn" @click="logout">退出</button></div></header>
    <main v-if="!user" class="login-wrap"><section class="login-hero"><p class="eyebrow">LEARNING OPERATIONS</p><h1>从一页教材<br>到一次有效学习。</h1><p>解析、审核、出题、考试与错题练习，都沿着原文出处有序推进。</p><div class="flow-mini"><span>教材 PDF</span><i>→</i><span>知识点</span><i>→</i><span>题库</span><i>→</i><span>考试</span></div></section><section class="login-card"><p class="eyebrow">WELCOME BACK</p><h2>{{ registerMode ? '创建考生账号' : '登录工作台' }}</h2><p class="muted">操作员和审核员使用服务端配置的账号。</p><label>用户名<input v-model="auth.username" autocomplete="username" /></label><label>密码<input v-model="auth.password" type="password" autocomplete="current-password" @keyup.enter="login" /></label><button class="primary full" :disabled="busy" @click="login">{{ registerMode ? '注册并登录' : '登录' }}</button><button class="text-btn full" @click="registerMode = !registerMode">{{ registerMode ? '已有账号？返回登录' : '考生注册新账号' }}</button><p v-if="error" class="alert error">{{ error }}</p></section></main>
    <div v-else class="workspace"><aside class="sidebar"><div class="side-label">工作区</div><button :class="{ active: tab === 'overview' }" @click="tab = 'overview'">◫ <span>总览</span></button><button :class="{ active: tab === 'books' }" @click="tab = 'books'">▤ <span>教材与学习</span></button><template v-if="isStaff"><button :class="{ active: tab === 'knowledge' }" @click="tab = 'knowledge'">◇ <span>知识点审核</span></button><button :class="{ active: tab === 'questions' }" @click="tab = 'questions'">☷ <span>试题审核</span></button><button :class="{ active: tab === 'papers' }" @click="tab = 'papers'">▣ <span>组卷与发布</span></button><button :class="{ active: tab === 'jobs' }" @click="tab = 'jobs'">◴ <span>后台任务</span></button></template><template v-else><button :class="{ active: tab === 'exam' }" @click="tab = 'exam'">✎ <span>参加考试</span></button><button :class="{ active: tab === 'results' }" @click="tab = 'results'">◈ <span>成绩与错题</span></button></template><div class="side-footer">PDF · 单选题 · 人工审核<br>第一版闭环</div></aside>
